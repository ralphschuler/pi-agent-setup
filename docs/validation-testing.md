...
`npm run test:ci` already includes `npm run check`, `npm test`, and coverage. Run `npm run docs:build` after docs, navigation, prompt, skill, or extension documentation changes. Run `npm run test:docker` for CI/Docker smoke-test parity when Docker is available.
