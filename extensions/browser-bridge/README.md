...
2. Run `/browser-bridge` or call `browser_bridge` with `action: "setup"`. The bridge server stays inactive until this step (or an agent browser action) activates it.
...
- `PI_BROWSER_BRIDGE_HOST` (default `127.0.0.1`; set `0.0.0.0` only when intentionally exposing to LAN)
