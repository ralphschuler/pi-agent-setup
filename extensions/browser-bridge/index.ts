...
const DEFAULT_HOST = process.env.PI_BROWSER_BRIDGE_HOST || "0.0.0.0";
...
function sendFrame(socket: import("node:net").Socket, payload: unknown) {
...
function wsAcceptKey(key: string) {
  return crypto
    .createHash("sha1")
...
export default function browserBridge(pi: ExtensionAPI) {
  ...
  async function startServer() {
    ...
    server = http.createServer((req, res) => {
      ...
      if (url.pathname === "/extension/background.js" || url.pathname === "/extension/manifest.json" || url.pathname === "/extension/popup.html" || url.pathname === "/extension/popup.js") {
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      ctx.ui.notify(`Browser bridge failed to listen on ${DEFAULT_HOST}:${port}: ${error instanceof Error ? error.message : String(error)}`, "error");
    }
  });
  ...
  pi.registerTool({
    ...
    promptSnippet: "Control a connected remote browser: status, navigate, click, type, read page text/html, evaluate JavaScript, screenshot, history, or reload.",
    promptGuidelines: [
