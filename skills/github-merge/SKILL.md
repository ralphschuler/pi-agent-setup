...
- Treat `/merge` invocation as approval to merge an unambiguous existing PR in the current repository when all safe-path checks pass.
...
- Do not ask extra confirmation on the normal unambiguous safe path.
...
5. Skip extra confirmation on the normal unambiguous safe path because invoking `/merge` is approval; use `human_in_loop` only when clarification or non-standard approval is needed.
...
- required clarification or non-standard approval is declined
